...
runtime-migrate:
	DATABASE_URL_SYNC=postgresql://driftgate:dev@localhost:55433/driftgate_runtime $(PYTHON) scripts/apply_runtime_migrations.py

simulate: runtime-up runtime-migrate
	TRACK_URL=http://localhost:8302/track \
	DATABASE_URL_SYNC=postgresql://driftgate:dev@localhost:55433/driftgate_runtime \
	SIM_REQUESTS=5000 \
	SIM_CONCURRENCY=200 \
	SIM_OUTPUT_PATH=docs/benchmarks/driftgate_simulation_5000.json \
	$(PYTHON) scripts/simulate_drift.py

demo:
	docker compose up -d --build
	until curl -sf http://localhost:8301/health >/dev/null; do sleep 2; done
	curl -X POST http://localhost:8301/api/monitor/run-once -H "X-DRIFTGATE-ADMIN-SECRET: $${ADMIN_SECRET:-dev-secret}"
